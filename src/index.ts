...
`);
  ...
  // Continue with MCP server startup (code below)
  startMcpServer();
}
...
function startMcpServer() {
  ...
  function validationError(message: string) {
    return {
      content: [
        { type: "text" as const, text: `Validation error: ${message}` },
      ],
      isError: true,
    };
  }
